...
async function _typeSentence(container, sentence, delay = 50) {
  for (let i = 0; i < sentence.length; i += 1) {
    await _pause(delay);
    container.append(sentence[i]);
  }
}
...
function _promptResponse(res) {
  ...
  let output = res.completion;

  if (res.completion) {
    // look for <output></output> and <score></score> tab
    let matched = res.completion.match(/<output>((.|\n)*)<\/output>/);

    if (matched) {
      output = matched[1].trim();

      matched = res.completion.match(/<score>((.|\n)*)<\/score>/);
      if (matched) {
        const score = Number(matched[1].trim());
...
export default class GenAITab extends BaseAnalysisTab {
  ...
  async onSubmitPrompt(sentenceEl) {
    ...
    return this.onPromptEnd(sentenceEl, params.prompt);
  }

  async onPromptEnd(sentenceEl, prompt) {
    sentenceEl.append('<br>----<br>');
    const _prompt = `(Original prompt: ${prompt})`;
    await _typeSentence(sentenceEl, _prompt, 30);
  }
