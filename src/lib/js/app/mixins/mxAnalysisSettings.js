...
const mxAnalysisSettings = (Base) => class extends Base {
  ...
  createRekognitionFeaturesForm() {
    ...
    const basicOptions = [
      ...
      [OPT_SEGMENT, Messages.Segment, Tooltips.Segment],
    ].map((x) =>
      this.createToggle(
  ...
  async restoreFactorySettings() {
    try {
      ...
      promises.push(this.removeGlobalAIOptions());

      await Promise.all(promises);

      // make sure certain contains default values
      this.aiOptions = JSON.parse(
        JSON.stringify(SolutionManifest.AIML)
      );

      // TODO: how to sync with the UI...
      this.updateControls();
    } catch (e) {
      console.error(e);
    } finally {
    ...
    const observer = new IntersectionObserver((entries) => {
      entries.forEach(async (entry) => {
        if (entry.intersectionRatio <= options.threshold[0]) {
          console.log(
            'mxAnalysisSettings.onPageInvisible',
